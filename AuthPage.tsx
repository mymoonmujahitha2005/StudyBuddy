  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (code === 'auth/invalid-credential') {
        setError(isLogin 
          ? 'Invalid email or password. Please check your credentials or click "Join the Moon" if you haven\'t created an account yet.' 
          : 'This account cannot be created. It may already exist or have been used with Google Access.');
      } else if (code === 'auth/user-not-found') {
        setError('No galactic explorer found with this email. Please sign up first!');
      } else if (code === 'auth/wrong-password') {
        setError('Incorrect password for this coordinates. Please try again.');
      } else if (code === 'auth/email-already-in-use') {
        setError('This email is already registered in our galaxy. Try signing in instead.');
      } else if (code === 'auth/weak-password') {
        setError('Password is too weak. It needs at least 6 characters to survive deep space.');
        ...
        setError(err.message || 'An unexpected error occurred during auth sync.');
      }
    } finally {
      setLoading(false);
    }
